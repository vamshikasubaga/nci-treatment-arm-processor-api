...
class MockCogService
  ...
  def self.perform
    Shoryuken.logger.info("#{self.class.name} | Mock COG service is Triggered to get the Latest TreatmentArm status")
  ...
  def self.status_update(cog_treatment_arm)
    begin
      ...
      Shoryuken.logger.error("#{self.class.name} | Failed to update TreatmentArm status from Mock COG with error #{error}::#{error.backtrace}")
    end
  end
